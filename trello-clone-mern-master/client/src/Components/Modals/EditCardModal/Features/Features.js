...
const Features = (props) => {
	...
	const handlePriorityChange = async (priority) => {
		try {
			// optimistic update
			dispatch({ type: 'card/updatePriority', payload: priority });
			// persist to server
			const axios = require('axios');
			await axios.put('/api/card/' + card.boardId + '/' + card.listId + '/' + card.cardId, { priority });
		} catch (error) {
	...
	const handleStatusChange = async (status) => {
		try {
			dispatch({ type: 'card/updateStatus', payload: status });
			const axios = require('axios');
			await axios.put('/api/card/' + card.boardId + '/' + card.listId + '/' + card.cardId, { status });
		} catch (error) {
	...
	const handleUpdateEffort = async (effort) => {
		try {
			dispatch({ type: 'card/updateEffort', payload: effort });
			const axios = require('axios');
			await axios.put('/api/card/' + card.boardId + '/' + card.listId + '/' + card.cardId, { effort });
		} catch (error) {
	...
	const handleTagsChange = async (tags) => {
		try {
			dispatch({ type: 'card/updateTags', payload: tags });
			const axios = require('axios');
			await axios.put('/api/card/' + card.boardId + '/' + card.listId + '/' + card.cardId, { tags });
		} catch (error) {
	...
	return (
		<Container>
			<FeatureContainer>
				<Title>Task</Title>
				<RowContainer>
					<PriorityBadge priority={card.priority} onChange={handlePriorityChange} />
					<TaskStatus status={card.status} onChange={handleStatusChange} />
				</RowContainer>
				<RowContainer style={{ marginTop: '8px' }}>
					<EffortEstimator
						estimatedEffort={card.effort.estimated}
						actualEffort={card.effort.actual}
						unit={card.effort.unit}
						aiGenerated={card.effort.aiGenerated}
						description={card.description}
						onUpdateEffort={handleUpdateEffort}
					/>
				</RowContainer>
				<RowContainer style={{ marginTop: '8px' }}>
					<TaskTags tags={card.tags || []} onTagsChange={handleTagsChange} />
				</RowContainer>
			</FeatureContainer>
