...
const cardsSlice = createSlice({
	...
	reducers: {
		...
		setCard: (state, action) => {
			...
			state.priority = action.payload.priority || 'medium';
			state.status = action.payload.status || 'todo';
			state.tags = action.payload.tags || [];
			state.effort = action.payload.effort || {
				estimated: null,
				actual: null,
				unit: 'hours',
				aiGenerated: false,
		...
		updateEffort: (state, action) => {
			state.effort = { ...state.effort, ...action.payload };
		},
