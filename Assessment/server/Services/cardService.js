...
const create = async (title, listId, boardId, user, callback) => {
	try {
		...
		// Create new card with TaskFlow fields
...
const deleteById = async (cardId, listId, boardId, user, callback) => {
	try {
		...
		// Update board task counts
		board.taskCount = Math.max(0, (board.taskCount || 1) - 1);
		if (result.status === 'completed') {
			board.completedTaskCount = Math.max(0, (board.completedTaskCount || 0) - 1);
		}
...
const updateDateCompleted = async (cardId, listId, boardId, user, completed, callback) => {
	try {
		...
		//Update date completed event and status
		const wasCompleted = card.date.completed;
		...
		// Update board completion stats
		if (completed && !wasCompleted) {
			board.completedTaskCount = (board.completedTaskCount || 0) + 1;
		} else if (!completed && wasCompleted) {
			board.completedTaskCount = Math.max(0, (board.completedTaskCount || 1) - 1);
		}
