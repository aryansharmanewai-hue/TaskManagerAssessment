...
const estimateTaskEffort = async (req, res) => {
	// Get the task description from request body
	...
	if (!description)
		return res
			.status(400)
			.send({ errMessage: 'Task description is required for estimation' });

	try {
		const aiEstimationService = require('../Services/aiEstimationService');
		const estimate = await aiEstimationService.generateAIEstimate(
			description,
			{ title }
		);
		...
		return res.status(500).send({
			errMessage: 'Failed to generate estimate',
			error: error.message,
		});
	}
};

const estimateBatchTasks = async (req, res) => {
	// Get array of tasks from request body
	...
	if (!Array.isArray(tasks) || tasks.length === 0)
		return res
			...
			.send({
				errMessage:
					'Array of tasks is required for batch estimation',
			});

	try {
		const aiEstimationService = require('../Services/aiEstimationService');
		const estimates = await aiEstimationService.estimateBatch(tasks);
		...
		return res.status(500).send({
			errMessage: 'Failed to generate batch estimates',
			error: error.message,
		});
	}
};
